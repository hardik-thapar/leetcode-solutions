class Solution {
    private int dfs(int i, int canBuy, int[] prices, int[][] memo){
        ...
        if(canBuy==1){
            memo[i][canBuy] = Math.max(dfs(i+1, 0, prices, memo)-prices[i], dfs(i+1, 1, prices, memo));
        }
        else memo[i][canBuy] = Math.max(dfs(i+2, 1, prices, memo)+prices[i], dfs(i+1, 0, prices, memo));
